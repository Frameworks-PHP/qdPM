...
<form class="form-horizontal" role="form"  id="ticketsReports" action="<?php echo url_for('ticketsReports/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
    ...
    <?php
      ...
      { 
        ...
        <div class="form-group">
        	...
        	<div class="col-md-9">          		              
            ' . select_tag('tickets_reports[projects_id]',explode(',',$v),array('choices'=>$choices,'multiple'=>true),array('class'=>'form-control','style'=>'height: 200px;'))  . '
        	</div>
        </div>
      ';  
